...
case class CRCModuleParams( // Note: do not put default value here
...
{
  val moduleName = "CRCCore"
}

object CRCModuleParams {
  implicit val rw: ReadWriter[CRCModuleParams] = macroRW

  def default(poly_coeff_p: Int, width_p: Int = 8) : CRCModuleParams =
    new CRCModuleParams(
...
class Axi4LiteCRC(p : CRCModuleParams, debugprint: Boolean = false)
  ...
  val dut = withReset(combinedReset) {Module(new CRCCore(width = p.width_p, accWidth = p.accWidth_p))}
...
object Axi4LiteCRCMain extends App {

  val p = checkParamEnv(
    CRCModuleParams.default(),
    "MAC_MODULE_PARAMS")

  EmitVerilog.generate(new Axi4LiteCRC(p, debugprint=true), p)
}
