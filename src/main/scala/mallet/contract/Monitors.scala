...
/** Auxiliary formal-only monitors: small registers that observe an interface so
  * that a temporal property becomes a plain combinational one.
  *
  * These helpers ALLOCATE HARDWARE, so they may only be called during module
  * elaboration (which is exactly when `ContractSet.properties` runs, via
  * `MalletProperties.contract`). Each returns a `maxPast == 0` property over the
  * monitor state it just built, so no `ltl.past` and no warm-up guard is needed
  * -- the monitor's own reset value handles the boundary.
  *
  * This is the machinery Phase 3b (role generation) reuses: a role like
  * `Commit(requires = [a_w, b_w])` will emit the same shape -- a sticky register
  * plus an assertion over it.
  */
object Monitors {
