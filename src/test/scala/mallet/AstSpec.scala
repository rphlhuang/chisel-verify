...
/** Unit tests for the correctness-critical parts of the AST: warm-up depth and
  * normalization. These are cheaper than round-tripping through firtool and
  * catch the failure modes that would silently weaken a property rather than
  * break the build.
  */
class AstSpec extends AnyFlatSpec with Matchers {

  // Terminals need real hardware to reference, so build them inside a throwaway
  // elaboration. The AST functions under test are pure, so this is just a way to
  // manufacture typed Bool/UInt handles.
  ...
  it should "be additive under nested past" in withSignals { (a, _, _) =>
    // CIRCT lowers nested past to a shiftreg feeding a shiftreg, so the depths
    // add. Taking the max here instead would under-guard by 1 and let the
    // property read uninitialised state.
  ...
  it should "take the max over BOTH sides of an implication" in withSignals { (a, b, _) =>
    // The trap: computing the guard depth from the antecedent alone lets a
    // past() in the consequent sample uninitialised shiftreg state while the
    // property is already live.
  ...
  it should "desugar a |=> b to a.past(1) |-> b" in withSignals { (a, b, _) =>
    // The semantic identity: a |=> b  ==  a.past(1) |-> b. Represented as a plain
    // Implies over a delay-1 Past, so no new AST node and no ltl.delay is ever
    // emitted.
  ...
  it should "push Not below Past so it never sits above a sequence" in withSignals { (a, _, _) =>
    // Property.not returns a Property, not a Sequence, so a Not above a Past
    // could not be used as an implication antecedent at all. Normalization must
    // eliminate that shape.
  ...
  it should "disclose the warm-up window whenever one is applied" in withSignals { (a, b, _) =>
    // The mask makes the property vacuously true for the first N cycles and can
    // hide a genuine reset-boundary bug. A report that does not say so claims
